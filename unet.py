...
def unet_model(pretrained_weights=None, input_size=(101,101,1)):
    ...
    conv10 = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer=initializer)(conv10)
    conv10 = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer=initializer)(conv10)
    up10 = Conv2D(32, 2, activation='relu', padding='same', kernel_initializer=initializer)\
        (UpSampling2D(size = (2,2))(conv10))
    merge10 = concatenate([conv1,up10], axis=3)

    conv11 = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer=initializer)(merge10)
    conv11 = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer=initializer)(conv11)
    conv11 = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer=initializer)(conv11)
    conv11 = Conv2D(2, 3, activation='relu', padding='same', kernel_initializer=initializer)(conv11)

    conv12 = Conv2D(1, 1, activation='sigmoid')(conv11)
    crop = Cropping2D(cropping=((14, 13), (14, 13)))(conv12)
